...
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
#[traced_test]
async fn openchannel_push_asset_amount_full_push() {
    initialize();

    let test_dir_node1 = format!("{TEST_DIR_BASE}openchannel_push_asset_amount_full_push/node1");
    let test_dir_node2 = format!("{TEST_DIR_BASE}openchannel_push_asset_amount_full_push/node2");
    let test_dir_node3 = format!("{TEST_DIR_BASE}openchannel_push_asset_amount_full_push/node3");
    let (node1_addr, _) = start_node(&test_dir_node1, NODE1_PEER_PORT, false).await;
    let (node2_addr, _) = start_node(&test_dir_node2, NODE2_PEER_PORT, false).await;
    ...
    wait_for_balance(node2_addr, &asset_id, 500).await;
