...
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
#[traced_test]
async fn fail_on_channel_close() {
    initialize();

    let test_dir_base = format!("{TEST_DIR_BASE}fail_on_channel_close/");
    let test_dir_node1 = format!("{test_dir_base}node1");
    let test_dir_node2 = format!("{test_dir_base}node2");
    let (node1_addr, _) = start_node(&test_dir_node1, NODE1_PEER_PORT, false).await;
    let (node2_addr, _) = start_node(&test_dir_node2, NODE2_PEER_PORT, false).await;
    ...
    let decoded = decode_ln_invoice(node1_addr, &invoice).await;
    let payments_2 = list_payments(node2_addr).await;
    let payment_2 = payments_2
        .iter()
        .find(|p| p.payment_hash == decoded.payment_hash)
        .unwrap();
    assert_eq!(payment_2.status, HTLCStatus::Pending);
    ...
    let payment_2 = payments_2
        ...
        .find(|p| p.payment_hash == decoded.payment_hash)
        .unwrap();
    assert_eq!(payment_2.status, HTLCStatus::Failed);
}
