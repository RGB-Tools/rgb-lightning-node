...
# Swap RGB for BTC (Sell Offer) Example
# This script demonstrates the inverse of the BTC-for-RGB swap,
# where a maker wants to sell their RGB asset for satoshis.
...
print_header "Swap RGB for BTC (Sell Offer) Example"
...
print_header "Issuing RGB Asset on Node 1 (Maker)"
asset_id=$(issue_rgb_asset 0 "RGB Sell Asset" 1000 0)
...
print_header "Maker Initiating Swap (RGB -> BTC)"
# Note: from_asset is the RGB asset, to_asset is null (BTC)
...
echo "6. Maker's BTC balance increased and RGB balance decreased"
